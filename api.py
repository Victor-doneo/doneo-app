...
def parse_pdf_bytes(pdf_bytes):
    """
    Extrait tournées et colis depuis les bytes d'une feuille CChezVous.
    Retourne un dict : code_complet -> {nom, date, colis: [{numero, type}]}
    """
    results = {}
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        ...
        for page in pdf.pages:
            ...
            # Ignorer les pages LIVRAISON (doublons)
...
def handler(event, context):
    ...
    # Vérification de la config DB
    if not DATABASE_URL:
        return resp(500, {'error': 'DATABASE_URL non configurée. Voir guide de déploiement.'})

    # Parsing du chemin
    raw_path = event.get('path', '/')
    api_path  = re.sub(r'^/api', '', raw_path)   # /api/tournees -> /tournees
